...
export const useCountDownTimer = ({ countDownTimeInMilliseconds, onEndOfTime }: CountDownTimerProps) => {
  const millisecond = 1000
  const timeUnit = 60
  const [countDownTime, setCountDownTime] = useState<number>(countDownTimeInMilliseconds)
  const [isStart, setIsStart] = useState(false)

  const startCountDownTime = () => setIsStart(true)

  const setCountDownTimeInMilliseconds = (countDownTimeInMillisecondsParam: number) =>
  ...
  const getCountdownData = (timePeriod: number): Time => {
    ...
    const minutes = Math.floor((timePeriod % (millisecond * timeUnit * timeUnit)) / (millisecond * timeUnit))
    const seconds = Math.floor((timePeriod % (millisecond * timeUnit)) / millisecond)
    return {
