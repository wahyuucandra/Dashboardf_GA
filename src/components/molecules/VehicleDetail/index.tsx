...
import { Photo, photosData } from './data'
...
function PhotoComponent({ closeClick = () => {} }: { closeClick?: any; appendClick?: any }) {
  const initialize = useRef<boolean>(false)
  ...
  return (
    <div className="relative pt-4">
      ...
      <div className="py-24">
        {photo?.image && <Image width={100} height={100} src={photo.image} className="w-full object-cover" alt="" />}{' '}
      </div>
      ...
      <div className="w-screen whitespace-nowrap overflow-x-auto pb-6">
        {photosData?.map((val, index) => (
          <div onClick={() => setPhoto(val)} key={index} className={`inline-block pr-0.5`}>
            <Image width={100} height={100} src={val?.image} className="w-16 h-16 object-cover" alt="" />
          </div>
        ))}
      </div>
    </div>
...
export default function ScheduleRoomDetail() {
  ...
  // const [selectedDate, setSelectedDate] = useState<{ start: DateInput; end: DateInput }>()
  // const [setSelectedTimes] = useState<TimeInput[]>()
    ...
    <>
      <div className="relative">
        <div className="sticky top-0 bg-white">
          ...
          <div className="grid grid-cols-3 gap-0.5">
            ...
            <Image
              ...
              alt="Booking Asset 2"
            />
            <div onClick={() => setIsPhotoOpen(true)} className="relative overflow-hidden">
              <Image
                width={100}
                height={100}
                className="object-cover w-full h-[74px]"
                src={photosData[3].image}
                alt="Booking Asset 3"
              />
              <div className="absolute bottom-0 w-full h-full bg-[#000000] opacity-50"></div>
              <div className="text-[10px] font-bold absolute flex top-0 items-center text-center justify-center w-full h-full text-white">
                +12 Foto Lainnya
              </div>
            </div>
          </div>
        ...
        <div className="px-3 pt-6 h-screen overflow-y-auto">
          <div className="mb-6">
            <div className="font-bold text-xl mb-3">Toyota Innova Reborn 2022</div>
          ...
          <div className="mb-6">
            <div className="text-sm">
              <div className="flex items-center space-x-2 mb-3">
                <IconPerson className="-mt-2 w-5 h-5"></IconPerson>
                <span>6 Orang</span>
              </div>
              <div className="flex items-center space-x-2 mb-3">
                <IconManual className="-mt-2 w-5 h-5"></IconManual>
                <span>Manual</span>
              </div>
              <div className="flex items-center space-x-2 mb-3">
                <IconPlateNumber className="w-5 h-5"></IconPlateNumber>
                <span>Ganjil</span>
              </div>
              <div className="flex items-center space-x-2 mb-3">
                <IconGasStation className="-mt-2 w-5 h-5"></IconGasStation>
                <span>Bensin</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2">
            <input id="check" type="checkbox" name="checkbox" className="w-5 h-5" />
            <label htmlFor="check" className="container text-sm">
          ...
          <div className="bg-white py-6 px-3 fixed z-[901] bottom-0 right-0 w-full border border-t">
            ...
            <button
              ...
              onClick={() => {}}
              type="button"
              className={` ${'bg-[#2C598D]'} h-11 w-full  text-[#ffffff] py-2.5 font-semibold rounded-lg`}
            >
              Booking Sekarang
            </button>
          </div>
      ...
      {/* Photo */}
      <Modal isOpen={isPhotoOpen} isFloating={false} backdropClick={() => setIsTimeOpen(false)}>
