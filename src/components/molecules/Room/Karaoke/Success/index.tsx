...
export function KaraokeSuccess () {
  ...
  const handleMappingDate = () => {
    if (scheduleForm?.startBookingDate && scheduleForm?.endBookingDate) {
      return `${moment(scheduleForm?.startBookingDate, 'YYYY-MM-DD').format('HH:mm:ss').toString()} - ${moment(
  ...
  const handleMappingTime = () => {
    if (scheduleForm?.startBookingTime && scheduleForm?.endBookingTime) {
      return `${moment(scheduleForm?.startBookingTime, 'HH:mm').format('DD/MM/YYYY').toString()} - ${moment(
        scheduleForm?.endBookingTime,
        'HH:mm'
      )
        .format('HH:mm:ss')
        .toString()}`
