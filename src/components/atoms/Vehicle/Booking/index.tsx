...
export function Booking({ acceptTerm }: Readonly<{ acceptTerm?: boolean }>) {
	...
	const handleBindDate = (
		input:
			| { start: DateInput | undefined; end: DateInput | undefined }
			| undefined
	) => {
	...
	const handleBindTime = (
		input:
			| { start: TimeInput | undefined; end: TimeInput | undefined }
			| undefined
	) => {
		...
		<>
			<div className='fixed z-[101] bg-white bottom-0 w-full border border-t-[#F6F6F6] shadow-[2px_4px_12px_0_rgba(0,0,0,0.1)] px-6 py-4 max-container'>
				<form onSubmit={handleSubmit(onSubmit)}>
					<div className='text-heading xs semibold-16 text-[#101010] mb-4'>
						Reservation Date
					</div>
