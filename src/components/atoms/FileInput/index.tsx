import React, { ChangeEvent, ComponentPropsWithoutRef, forwardRef, useEffect, useRef, useState } from 'react'
    ...
    const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => {
      ...
      if (file) {
        if (!accept.includes(file.type)) {
          toast.error(`File must be one of the following types: ${accept}`)
          return
        }
            ...
            <div
              className={joinClass(
                `items-center flex mx-2 line-clamp-2 justify-center h-[calc(100%-8px)] bg-[#fafafa] text-left ${
                  selectedDocument ? 'w-[calc(100%-80px)] pr-10' : 'w-[calc(100%-8px)]'
                }`,
